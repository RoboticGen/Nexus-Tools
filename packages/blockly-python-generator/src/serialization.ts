...
/** Returns the active main workspace. Uses Blockly.getMainWorkspace() to avoid stale refs from React re-renders. */
export function getActiveWorkspace(): Blockly.WorkspaceSvg {
  return Blockly.getMainWorkspace() as Blockly.WorkspaceSvg;
}

/** Saves the state of the workspace to browser's local storage. @param workspace Blockly workspace to save. */
export const save = function(workspace: Blockly.Workspace) {
  const data = Blockly.serialization.workspaces.save(workspace);
  window.localStorage?.setItem(storageKey, JSON.stringify(data));
};

/** Loads saved state from local storage into the given workspace. @param workspace Blockly workspace to load into. */
...
export const importJson = function(json: any): boolean {
  try {
    ...
    // recompute the inverse screen CTM (screen ↔ workspace coordinate matrix). Blockly normally only does this on the first mouse event, which is exactly why clicking once "fixed" the import before this change.
