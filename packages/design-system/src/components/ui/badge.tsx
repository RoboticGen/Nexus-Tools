...
import type { Appearance, Tone } from "@nexus-tools/design-system/lib/tone"

const badgeVariants = cva(
  // `border` sets width only — the colour comes from the variant, or for a toned badge from the `[data-appearance]` rules in the theme. Putting `border-transparent` in the base would break toned badges: it compiles to a utility, and Tailwind's utilities layer wins over the components layer the tone rules live in.
  "group/badge inline-flex h-5 w-fit shrink-0 items-center justify-center gap-1 overflow-hidden rounded-4xl border px-2 py-0.5 text-xs font-medium whitespace-nowrap transition-all focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 has-data-[icon=inline-end]:pr-1.5 has-data-[icon=inline-start]:pl-1.5 aria-invalid:border-destructive aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 [&>svg]:pointer-events-none [&>svg]:size-3!",
  {
    variants: {
      variant: {
        ...
        // Stock shadcn pairs `text-destructive` with a 10% tint of itself, which lands at 4.13:1 on our palette — a real AA failure the a11y test caught. --tone-danger-contrast is the same hue derived to be readable on exactly that tint.
        destructive:
        ...
        /** Contributes no colour at all — `data-tone` and `data-appearance` paint it. Selected automatically when `tone` is passed; not meant to be set by hand. */
        toned: "",
      },
    },
    defaultVariants: {
...
type BadgeProps = React.ComponentProps<"span"> &
  Omit<VariantProps<typeof badgeVariants>, "variant"> & {
    ...
    >
    asChild?: boolean
    /** Semantic weight. Setting this switches the badge onto the tone system and `variant` is ignored — the two are alternative ways to colour it. */
    tone?: Tone
...
/** A label for status, category or count. Two ways to colour it, deliberately kept separate: - `variant` — the shadcn surface roles (`default`, `secondary`, `outline`…). */
function Badge({
  className,
  variant,
  tone,
