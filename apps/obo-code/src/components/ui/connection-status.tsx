...
/**
 * Device connection indicator and its connect/disconnect control.
 *
 * Both Nexus-Tools output panels rebuild this: an `isConnected` ternary
 * choosing between a primary "Connect Device" and a danger "Disconnect", with
 * the state itself communicated only by which button is showing. That leaves
 * `connecting`, `unsupported` and `error` with nowhere to go — the apps
 * currently route them through toasts, so the panel shows "Connect Device"
 * while Web Serial is unavailable and the button can never work.
 *
 * The five states are explicit and tone-driven, so the indicator uses the same
 * scale as StatusBadge rather than a second set of colours.
 */
  ...
  return (
    ...
    >
      <span
        ...
        // A live region: the connection can drop without the user doing
        // anything, and that is worth announcing. Polite, not assertive —
        ...
        <>
          {connectable && onConnect && (
            <Button
              ...
              variant="outline"
              onClick={onConnect}
              // `unsupported` deliberately renders no button at all rather
              // than a disabled one: there is nothing the user can do to make
              // it work, and a greyed button invites them to keep trying.
              disabled={disabled}
            >
