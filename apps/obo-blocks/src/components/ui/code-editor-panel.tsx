...
type CodeEditorPanelProps = Omit<React.ComponentProps<"div">, "title"> & {
  ...
  /**
   * Marks the buffer as not editable — obo-blocks' generated-Python view.
   * Adds a badge to the toolbar and `aria-readonly` to the panel; the editor
   * in the slot still has to enforce it.
   */
  readOnly?: boolean
...
/**
 * Panel wrapper for a code editor: toolbar → file tabs → editor slot.
 *
 * The editor is a `children` slot rather than a built-in Monaco mount, because
 * the design system does not depend on `@monaco-editor/react` — the consumer
 * brings their own editor and drops it in. The only job here is the *chrome*:
 * a named toolbar, real file tabs, and the tokens.
 *
 * Tabs can close, rename and add. `SharedCodeEditor`'s rename is double-click
 * only, which a keyboard user cannot reach at all — F2 (and Delete/Backspace
 * for closing) do the same thing here, alongside the mouse controls. A close
 * `<button>` cannot live inside the tab trigger (`TabsTrigger` is itself a
 * button, and a button-in-a-button is invalid HTML axe will flag) or as any
 * other descendant of `TabsList` (`role="tablist"` only permits `role="tab"`
 * descendants — axe's `aria-required-children` flags a nested close button
 * even behind a plain wrapper `<div>`). It renders as a sibling of `TabsList`
 * instead, repositioned onto the tab row with CSS `order`, and disappears
 * once one tab is left — the original's min-1 guard.
 */
function CodeEditorPanel({
  ...
  // Rename's draft text is local UI state for the same reason Terminal's
  // scroll-pin and DeviceSidebar's expand state are: no consumer wants to own
  // a half-typed value between keystrokes.
  ...
  return (
    <Panel
      ...
      // Not `aria-readonly`: that is only allowed on widget roles, and this is
      // a plain container. The badge in the toolbar carries the meaning; the
      // editor in the slot is what actually enforces it.
