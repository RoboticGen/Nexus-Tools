 ...
 * Application top bar.
 *
 * obo-code and obo-blocks each carry their own `navbar.tsx`. The 35 differing
 * lines are entirely incidental — `navbar` vs `nav-bar`, `<img>` vs
 * `<Image>`, `logo` vs `academy-logo` — and every one of the differences is a
 * class name feeding a per-app stylesheet. The structure and the Keycloak
 * logout handler are identical.
 *
 * The logout handler is deliberately **not** here: it is next-auth and
 * Keycloak specific, so it stays in the app and arrives through `actions`. A
 * design system that imports `next-auth/react` cannot be installed by an app
 * that does not use it — the same mistake `sonner.tsx` shipped with when it
 * imported `next-themes`.
 */
function AppHeader({
  brand,
  nav,
  ...
  return (
    <header
      ...
      className={cn(
        ...
        // z-sticky (100) from the ladder, not a literal: this has to sit under
        // every overlay but above page content.
